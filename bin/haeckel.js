...
var Haeckel;
(function (Haeckel) {
    var PhyloBuilder = (function () {
        function PhyloBuilder() {
            this.dagBuilder = new Haeckel.DAGBuilder();
        }
        PhyloBuilder.prototype.addPhylogeny = function (g) {
            var _this = this;
            Haeckel.ext.each(g.arcs, function (arc) {
                _this.addPrecedence(arc[0], arc[1]);
            }, this);
            return this;
        };

        PhyloBuilder.prototype.addPrecedence = function (prc, suc) {
            var _this = this;
            Haeckel.ext.each(prc.units, function (prcUnit) {
                Haeckel.ext.each(suc.units, function (sucUnit) {
                    _this.dagBuilder.addArc(prcUnit, sucUnit);
                }, _this);
            }, this);
            return this;
        };

        PhyloBuilder.prototype.addTaxon = function (taxon) {
            var _this = this;
            Haeckel.ext.each(taxon.units, function (unit) {
                _this.dagBuilder.addVertex(unit);
            }, this);
            return this;
        };

        PhyloBuilder.prototype.build = function () {
        ...
        PhyloBuilder.prototype.removePhylogeny = function (g) {
            var _this = this;
            Haeckel.ext.each(g.arcs, function (arc) {
                _this.removePrecedence(arc[0], arc[1]);
            }, this);
            return this;
        };

        PhyloBuilder.prototype.removePrecedence = function (prc, suc) {
            var _this = this;
            Haeckel.ext.each(prc.units, function (prcUnit) {
                Haeckel.ext.each(suc.units, function (sucUnit) {
                    _this.dagBuilder.removeArc(prcUnit, sucUnit);
                }, _this);
            }, this);
            return this;
        };

        PhyloBuilder.prototype.removeTaxon = function (taxon) {
            var _this = this;
            Haeckel.ext.each(taxon.units, function (unit) {
                _this.dagBuilder.removeVertex(unit);
            }, this);
            return this;
        };

        PhyloBuilder.prototype.reset = function () {
            this.dagBuilder.reset();
            return this;
        };
        return PhyloBuilder;
    })();
    Haeckel.PhyloBuilder = PhyloBuilder;
})(Haeckel || (Haeckel = {}));
var Haeckel;
(function (Haeckel) {
...
(function (Haeckel) {
    ...
    var StateRenderer = (function () {
        ...
        StateRenderer.prototype.render = function (element) {
            var _this = this;
                ...
                for (var column = this.minColumn + 1; column <= this.maxColumn; ++column) {
                    ...
                    if (lastTop !== columnY.top) {
                        if (Math.abs(lastTop - columnY.top) <= this.cornerRadius * 2) {
                            ...
                            d += 'Q' + [lastArea.right, lastTop, lastArea.right, lastTop + this.cornerRadius].join(' ') + 'V' + (columnY.top - this.cornerRadius);
                            d += 'Q' + [lastArea.right, columnY.top, lastArea.right + this.cornerRadius, columnY.top].join(' ');
                        } else {
                            d += 'h' + this.cornerRadius + 'Q' + [lastArea.right + this.cornerRadius, lastTop, lastArea.right + this.cornerRadius, lastTop - this.cornerRadius].join(' ') + 'V' + (columnY.top + this.cornerRadius);
            ...
            d += 'Q' + [area.right, columnY.top, area.right, columnY.top + this.cornerRadius].join(' ') + 'V' + (columnY.bottom - this.cornerRadius);

            d += 'Q' + [area.right, columnY.bottom, area.right - this.cornerRadius, columnY.bottom].join(' ') + 'H' + (area.left + this.cornerRadius);
            if (this.minColumn !== this.maxColumn) {
                for (column = this.maxColumn - 1; column >= this.minColumn; --column) {
                    ...
                    if (lastBottom !== columnY.bottom) {
                        if (Math.abs(lastBottom - columnY.bottom) <= this.cornerRadius * 2) {
                            ...
                            d += 'H' + lastArea.left + 'Q' + [lastArea.left - this.cornerRadius, lastBottom, lastArea.left - this.cornerRadius, lastBottom + this.cornerRadius].join(' ') + 'V' + (columnY.bottom - this.cornerRadius);
                            d += 'Q' + [lastArea.left - this.cornerRadius, columnY.bottom, lastArea.left - this.cornerRadius * 2, columnY.bottom].join(' ');
                        } else {
                            d += 'Q' + [lastArea.left, lastBottom, lastArea.left, lastBottom - this.cornerRadius].join(' ') + 'V' + (columnY.bottom + this.cornerRadius);
                            d += 'Q' + [lastArea.left, columnY.bottom, lastArea.left - this.cornerRadius, columnY.bottom].join(' ');
                        }
                    }
            ...
            if (this.label) {
                ...
                this.label.split(/\n+/g).forEach(function (word, index) {
                    label.child(Haeckel.SVG_NS, 'tspan').text(word).attrs(Haeckel.SVG_NS, {
                        x: (area.left + _this.cornerRadius * 4) + 'px',
                        dy: (index > 0) ? _this.fontSize + 'px' : '0'
                    });
                });
            }
    ...
    var CharacterMatrixChart = (function () {
        function CharacterMatrixChart() {
            this.area = Haeckel.EMPTY_SET;
            this.characters = [];
        ...
        CharacterMatrixChart.prototype.render = function (parent) {
        ...
        CharacterMatrixChart.prototype._getArea = function (row, column) {
            ...
            return Haeckel.rec.createFromCoords(x, y, x + w, y + h);
        };

        CharacterMatrixChart.prototype.renderCharacter = function (element, character, row) {
            var columns = this.taxa.length;
            if (!(columns > 0)) {
                return;
            }
            var g = element.child(Haeckel.SVG_NS, 'g');
            var statesGroup = g.child(Haeckel.SVG_NS, 'g');
            var numStates = Haeckel.bit.size(character.domain);
            if (!(numStates > 0)) {
                return;
            }
            var cells = new Array(columns);
            var unknownsBuilder = new Haeckel.ExtSetBuilder();
            for (var column = 0; column < columns; ++column) {
                ...
                if (states === null) {
                    ...
                    for (var state = 0; state < numStates; ++state) {
                        if (Haeckel.bit.contains(states, state)) {
                            cell.push(state);
                        }
                    }
                    cells[column] = cell;
            ...
            stateRenderers.forEach(function (renderer) {
                return renderer.render(statesGroup);
            });
            var unknowns = unknownsBuilder.build();
            if (!unknowns.empty) {
                var unknownsGroup = g.child(Haeckel.SVG_NS, 'g');
                for (column = 0; column < columns; ++column) {
                    if (!Haeckel.ext.contains(unknowns, column)) {
                        continue;
                    }
                    var start = column;
                    while (column < columns && Haeckel.ext.contains(unknowns, column + 1)) {
                        ++column;
                    }
        ...
        return CharacterMatrixChart;
    })();
    Haeckel.CharacterMatrixChart = CharacterMatrixChart;
})(Haeckel || (Haeckel = {}));
var Haeckel;
(function (Haeckel) {
...
(function (Haeckel) {
    (function (chr) {
        function createInt(criterion, combine, readStates, writeStates, label, labelStates, stateLabels) {
            ...
            c.intersect = Haeckel.intersector(Haeckel.ist.intersect, Haeckel.EMPTY_SET);
...
(function (Haeckel) {
    (function (fig) {
        ...
        function render(figure, document, files, serializer) {
            ...
            var elementBuilder = new Haeckel.ElementBuilder(document, Haeckel.SVG_NS, 'svg').attrs({
