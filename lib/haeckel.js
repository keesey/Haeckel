...
if (process) {
    ...
    var fs = require('fs'), figureFilename = process.argv[2], figure;
    fs.readFile(figureFilename, 'utf8', function (error, data) {
        if (error) {
        ...
        var figure = eval(data), xmldom = require("xmldom"), parser = new xmldom.DOMParser(), doc = parser.parseFromString('<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="' + figure.width + '" height="' + figure.height + '"></svg>', 'image/svg+xml'), builder = new Haeckel.ElementBuilder(doc, doc.documentElement);
