...
if (process) {
    ...
    var SVG_MIME_TYPE = 'image/svg+xml', fs = require('fs'), figureFilename = process.argv[2], options = { encoding: 'utf8', flag: 'r' }, figure = eval(fs.readFileSync(figureFilename, options)), path = require('path'), dir = path.dirname(figureFilename), sources = {}, assets = {}, filename, i, n;
    if (figure.sources) {
        var data = {};
    ...
    var xmldom = require("xmldom"), parser = new xmldom.DOMParser(), serializer = new xmldom.XMLSerializer();
    if (figure.assets) {
        ...
        if (figure.assets.svg) {
            for (i = 0, n = figure.assets.svg.length; i < n; ++i) {
                ...
                assets[filename] = serializer.serializeToString(parser.parseFromString(fs.readFileSync(path.join(dir, filename), options), SVG_MIME_TYPE).documentElement);
            }
        }
    }
    var doc = parser.parseFromString('<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="' + figure.width + '" height="' + figure.height + '"></svg>', SVG_MIME_TYPE), builder = new Haeckel.ElementBuilder(doc, doc.documentElement);
